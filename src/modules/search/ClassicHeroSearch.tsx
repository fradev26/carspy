...
<SelectValue placeholder="Elke prijs" />
      ...
      {/* Mobile: compacte trigger die een sheet opent met filters */}
        ...
        <Sheet open={sheetOpen} onOpenChange={setSheetOpen}>
          <div className="flex items-center gap-2 rounded-md bg-white p-2 shadow-sm">
            <SheetTrigger asChild>
              <button
                type="button"
                className="flex flex-1 items-center justify-between gap-2 rounded-sm bg-background px-3 h-11 text-sm font-medium text-foreground/80 hover:bg-muted/60 focus-ring text-left"
              >
                <span className="truncate">
                  {[brand || 'Alle merken', model, maxPrice ? `Tot € ${Number(maxPrice).toLocaleString('nl-NL')}` : null]
                    .filter(Boolean)
                    .join(' · ')}
                </span>
            ...
            <Button
              type="button"
              onClick={handleSearch}
              className="h-11 bg-primary hover:bg-primary/90 text-primary-foreground px-4 gap-2 text-sm font-semibold"
            >
              <Search className="h-4 w-4" />
              Zoeken
            </Button>
...
<SelectValue placeholder="Elke prijs" />
              ...
              <div className="flex gap-2 pt-2">
                <button
                  type="button"
                  onClick={() => { setSheetOpen(false); handleMoreFilters(); }}
                ...
                <Button
                  type="button"
                  onClick={() => { setSheetOpen(false); handleSearch(); }}
